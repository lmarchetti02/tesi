...
{
    for (int i = 0; i < list_info.size(); i++) {
        G4cout << "FLUORO EvID=" << evid << " phID=" << list_info[i]->particle_id << " volID=" << list_info[i]->volume_id << " ph_ene=" << list_info[i]->energy << " ene_loss=" << list_info[i]->eneLoss
               << " n.iter=" << list_info[i]->iter_found << G4endl;
    }
}
