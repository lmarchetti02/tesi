 ...
 */
PixelsHit::PixelsHit(const PixelsHit &otherHit)
    : G4VHit()
{
 ...
 * Overloading of the `==` operator.
 */
G4bool PixelsHit::operator==(const PixelsHit &otherHit) const { return (this == &otherHit) ? true : false; }
...
{
    if (!MyHitAllocator) MyHitAllocator = new G4Allocator<PixelsHit>;
 ...
 * Overloading of the `new` operator
 */
void PixelsHit::operator delete(void *aHit) { MyHitAllocator->FreeSingle((PixelsHit *)aHit); }
